...
int baronCardEffect(int card, int choice1, struct gameState *state, int currentPlayer) {
    ...
    else
    {
        if (supplyCount(estate, state) > 0)
        {
            ...
            state->supplyCount[estate]++; //Increment Estates //*BUG* -- to ++
            if (supplyCount(estate, state) == 0)
            {
                isGameOver(state);
            }
...
int minionCardEffect(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer){
    ...
    //discard card from hand
    discardCard(handPos, currentPlayer, state, 1); //*BUG* 0 to 1

    if (!choice1) //*BUG* choice1 to !choice1
    {
        state->coins = state->coins + 2;
    }
    else if (choice2) //discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
    {
        discardHandDrawFour(state, handPos, currentPlayer);
        ...
        for (int i = 0; i < state->numPlayers; i++)
        {
            if (i != currentPlayer)
            {
                ...
                {
                    discardHandDrawFour(state, handPos, i);   
                }
            }
        }
    }
...
int ambassadorCardEffect(int choice1, int choice2, struct gameState *state, int handPos, int *bonus, int currentPlayer) {
    ...
    state->supplyCount[state->hand[currentPlayer][choice1]] -= choice2; //*BUG* += to -=

    //each other player gains a copy of revealed card
    for (int i = 0; i < state->numPlayers; i++)
    {
        if (i == currentPlayer) // *BUG* changed != to == *BUG*
        {
...
int tributeCardEffect(struct gameState *state,int currentPlayer, int nextPlayer, int* tributeRevealedCards) {
    ...
    else
    {
        if (state->deckCount[nextPlayer] == 0)
        {
            ...
            shuffle(currentPlayer, state); //Shuffle the deck //*BUG* nextPlayer to currentPlayer
        }
        tributeRevealedCards[0] = state->deck[nextPlayer][state->deckCount[nextPlayer] - 1];
...
int mineCardEffect(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer) {
    ...
    gainCard(choice1, state, 2, currentPlayer); //*BUG* choice2 to choice1
